...
class Decomposer(object):
    ...
    def __init__(self, query, config, tempType="MULDER", joinstarslocally=True, sparql1_1=False):
        self.tempType = tempType
        self.sparql1_1 = sparql1_1
        if not self.sparql1_1:
            from DeTrusty.Sparql.Parser import queryParser
    ...
    def decompose(self):
        groups = self.decomposeUnionBlock(self.query.body) if not self.sparql1_1 else self.query.body
        if groups is None or not groups:
            return None

#        print("type(groups):", type(groups))
#        for jb in groups.triples:
#            print("type(jb):", type(jb))
#            for leaf in jb.triples:
#                print("type(leaf):", type(leaf))
